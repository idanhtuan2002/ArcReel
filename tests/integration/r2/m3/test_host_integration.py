...
def setup_sh01(tmp_path):
    ...
    resource_type = (
        "video"
        if "video" in VersionManager.RESOURCE_TYPES
        else sorted(VersionManager.RESOURCE_TYPES)[0]
    )
...
def test_real_host_b_c_d_master_semantics_and_restart(tmp_path):
    ...
    assert (
        host.snapshot(prepared.shot.id)
        .approved_master.selected_candidate_id
        == b.candidate.id
    )
    assert sha(host.current_file_for(prepared.shot.id)) == produced_b.sha256
    ...
    assert (
        restarted.snapshot(prepared.shot.id)
        .approved_master.selected_candidate_id
        == d.candidate.id
    )
    assert (
        sha(restarted.current_file_for(prepared.shot.id))
        == produced_d.sha256
    )
