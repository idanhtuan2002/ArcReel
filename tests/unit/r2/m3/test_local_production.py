...
def prepared(reused_asset_ref: str | None):
    ...
    shots = GoldenAOpenMontageAdapter(
...
def test_reuse_production_preserves_source_and_emits_valid_mp4(tmp_path):
    ...
    assert asset.path.is_file() and asset.path.stat().st_size > 0
    subprocess.run(
        [
            "ffprobe", "-v", "error",
            "-select_streams", "v:0",
...
def test_deterministic_output_is_byte_stable_for_same_execution_variant(tmp_path):
    sh02 = prepared(
        "r2/m3/fixtures/assets/reused_terminal_frame.svg"
    )[1]
    producer = GoldenALocalProducer()
    a = producer.produce(
        sh02, work_dir=tmp_path / "a", execution_variant="same"
    )
    b = producer.produce(
        sh02, work_dir=tmp_path / "b", execution_variant="same"
    )
...
def test_execution_variant_changes_execution_fingerprint_not_method(tmp_path):
    ...
    producer = GoldenALocalProducer()
    a = producer.produce(
        sh02, work_dir=tmp_path / "a", execution_variant="A"
    )
    b = producer.produce(
        sh02, work_dir=tmp_path / "b", execution_variant="B"
    )
