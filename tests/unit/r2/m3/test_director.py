...
def test_fixture_director_emits_three_scenes_and_four_shots():
    ...
    assert len(result.scenes)==3 and len(result.shots)==4
...
def test_direction_preserves_lineage_and_factual_basis():
    ...
    section_ids={s.id for s in script.sections}; scene_ids={logical_id(s) for s in result.scenes}
    for scene in result.scenes:
        if hasattr(scene,"source_unit_refs"): assert set(scene.source_unit_refs) <= section_ids
        if hasattr(scene,"content_basis"): assert scene.content_basis.basis_type is ContentBasisType.FACTUAL
    for shot in result.shots:
        assert shot.scene_id in scene_ids
        if hasattr(shot,"content_basis"): assert shot.content_basis.basis_type is ContentBasisType.FACTUAL
