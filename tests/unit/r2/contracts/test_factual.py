...
def test_research_pack_deduplicates_refs_deterministically():
    pack = ResearchPack(
        ...
        topic="Git staging",
        scope="Golden A",
        source_refs=["SRC-2", "SRC-1", "SRC-2"],
        evidence_refs=["E2", "E1", "E2"],
        synthesis="Frozen factual snapshot.",
        uncertainties=[],
        open_questions=[],
        provenance=provenance(),
    )
...
def test_source_and_evidence_require_aware_timestamps():
    ...
    with pytest.raises(ValidationError):
        SourceRecord(
            ...
            origin="https://git-scm.com/docs/git-add",
            captured_at=naive,
    ...
    with pytest.raises(ValidationError):
        EvidenceRecord(
            id="E1",
            source_ref="SRC-1",
            locator="snapshot:git-add",
            excerpt_or_structured_fact="git add updates the index",
            evidence_type="TEXT",
            captured_at=naive,
            provenance=provenance(),
        )
...
def test_claim_status_wire_semantics_are_stable():
    ...
    claim = Claim(
        id="CLAIM-WIRE",
        version=1,
        statement="wire semantics",
        evidence_refs=["EV-WIRE"],
        confidence=1.0,
        status=ClaimStatus.VERIFIED,
        provenance={"actor": "test"},
    )

    assert claim.model_dump(mode="json")["status"] == "VERIFIED"
