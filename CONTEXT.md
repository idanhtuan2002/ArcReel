...
**叙事正典（Canon）**：
一个虚构项目在某个已提交版本中成立的权威事实、事件与语义实体集合。
_Avoid_: 把草稿、创作计划、模拟结果或制作资产当作已经成立的叙事真相。

**正典分支（CanonBranch）**：
一条独立演进的叙事连续性；叙事分支固定引用一个父分支版本，父分支后续推进不会暗中改变它。
_Avoid_: 改编分支、复制整份父正典、跟随父分支当前版本的浮动视图。

**正典版本（CanonVersion）**：
一次通过正式正典事务产生的不可变叙事真相版本。
_Avoid_: 可编辑快照、草稿版本、分支当前指针。

**正典变更提案（CanonDelta）**：
针对一个明确基础正典版本、准备由正式事务校验并提交的一组显式操作；提案本身不是正典。
_Avoid_: NarrativeChangeSet、直接数据库更新、已经提交的正典版本。

**正典解析视图（ResolvedCanonView）**：
把分支固定的父版本与该分支已提交变更合并后得到的确定性读取结果；它可由权威历史重新生成。
_Avoid_: 独立真相源、可绕过正典事务修改的物化副本。

**正典事务（Canon transaction）**：
对正典变更提案执行基础版本、约束与批准校验，并以全有或全无方式产生下一正典版本的唯一正式提交过程。
_Avoid_: 自动接受、局部写入、由写作工具或 Agent 直接修改正典。
