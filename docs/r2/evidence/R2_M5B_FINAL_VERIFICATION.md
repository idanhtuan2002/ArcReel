...
1. **Per-checkpoint Codex review flow collapsed; replaced by a self-review.** The plan's three evidence-only handoff commits never happened: the operator directed Claude Code to implement M5B-1, M5B-2, and M5B-3 in one session (Codex unavailable — usage limit). Codex stayed unavailable for the re-review of `3085d14e`, so a two-axis self-review (`/code-review`) was run instead and its outcome recorded per checkpoint in `R2_M5B_1_REVIEW.md` / `R2_M5B_2_REVIEW.md` / `R2_M5B_3_REVIEW.md` (self-review verdict, not a Codex verdict). Those three docs are not separate handoff commits — the verified→handoff diff chain the plan specified is retired. The frozen heads `2a99ede1` / `3d206f15` / `f848c085` (plus the post-`3085d14e` fix commits) remain valid targets for a later Codex re-review.
