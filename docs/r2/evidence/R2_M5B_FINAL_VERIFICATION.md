...
1. **Per-checkpoint Codex review flow collapsed; replaced by an accepted self-review.** The plan's three evidence-only handoff commits never happened: the operator directed Claude Code to implement M5B-1, M5B-2, and M5B-3 in one session (Codex unavailable — usage limit). Codex stayed unavailable, and the operator then waived the Codex re-review: a two-axis self-review (`/code-review`) is the review of record, its outcome recorded per checkpoint in `R2_M5B_1_REVIEW.md` / `R2_M5B_2_REVIEW.md` / `R2_M5B_3_REVIEW.md`. Those three docs are not separate handoff commits — the verified→handoff diff chain the plan specified is retired. The frozen heads `2a99ede1` / `3d206f15` / `f848c085` and the post-`3085d14e` fix commits are still available should a Codex pass be run later, but no such pass is pending.
...
10. **Standards-axis smell judgement-calls left open (deferred cleanup).** The self-review's Standards axis raised ~10 non-blocking Fowler-smell judgement-calls, none a documented-standard breach: a canonical-JSON→sha256 helper reimplemented four times (`_canonical_json_bytes` / `_canonical_bytes` / `_sha` / `_sha256_ref`) instead of the exported `canonical_json_bytes`; the "known Canon schema selectors" set duplicated across `hashing.py` / `integrity.py` / `schema_upgrade.py` / `canon_resolver.py` as bare literals; a half-open-interval predicate repeated as `_active_at` / `_is_active` / `_point_in` / `_covers`; `_aware` / `_aware_optional` / `_sorted_unique` re-pasted into `contracts/narrative_context.py` and `contracts/narrative_plan.py`; `r2/narrative/validation.py` accreting M5A-compat, M5B epistemic/temporal, and SceneContract rules in one file (+585 in the M5B range); `validate_canon_candidate` / `next_scene_version` / `SceneVersionCheck.ok` / `NarrativePlanNodeKind` / `RetrievalCandidate.score` with no production caller or effect. Deferred as a single `/simplify` pass rather than folded into the self-review fixes, since each edit reaches into several already-verified files.
